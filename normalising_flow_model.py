...
# Normalising Flow model using Zuko's NSF with coupling transforms (passes=2)
# This is akin to RealNVP's coupling-layer architecture but with neural spline
# transforms instead of simple affine transforms.
...
class NormalisingFlowModel(nn.Module):
    def __init__(self, hidden_layers, input_dim, output_dim,
                 ...
                 num_transforms=3, sigma=0.1, dropout=0.2, tail_bound=5.0,
                 activation=nn.GELU):
        """
        Normalising Flow Model using Zuko's NSF with coupling transforms.

        Args:
            hidden_layers: List of hidden layer widths for the coupling networks
            ...
            num_transforms: Number of coupling transform layers
            sigma: Noise level for dequantisation (unused by the flow itself,
                   kept for API consistency with FlowMatchingModel)
            dropout: Dropout for coupling networks (unused by Zuko's built-in
                     architectures, kept for API consistency)
            activation: Activation function for the coupling networks (default: nn.GELU)
        """
        super().__init__()
        ...
        # passes=2 makes each transform a COUPLING layer (RealNVP-style)
        ...
        if "tail_bound" in inspect.signature(zuko.flows.NSF).parameters:
            nsf_kwargs["tail_bound"] = tail_bound

        self.flow = zuko.flows.NSF(**nsf_kwargs)
    ...
    def training_step(self, context, x1_raw):
        ...
        Backpropagation runs through the NSF coupling network parameters.
        """
        B, M, D = x1_raw.shape
