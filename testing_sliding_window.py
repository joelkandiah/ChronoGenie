...
def get_sliding_window_predictions(
    ...
    msoa_name_map = dataset_directory.geographies
    ...
    # Pre-build mappings for saving
    ctx_to_pred_idx = {c_idx: prediction_names.index(c_name) for c_idx, c_name in enumerate(context_names) if c_name in prediction_names}

    # Tracking simulation-level metrics across steps
    # Structure: sim_id -> timestep -> samples, gt, scores
    ...
    for current_test_window in range(autoregressive_window_size):
        ...
        for sim_id in dataset_directory.test_sims:
            ...
            # We shape outputs to match the expectations of proper_scoring loops: [S, M, V] & [M, V]
    ...
    for sim_id in dataset_directory.test_sims:
        ...
        # Chronos generates results one horizon step at a time
        for pred_time_idx, data in sim_data_tracker[sim_id].items():
            ...
            samples_win = data["samples"].unsqueeze(0)  # [1, S, M, V] (1 timestep window chunk)
            ...
            # Unified stack arrays to reduce device transfers
        ...
        # Save timing summary for the individual simulation pipeline
