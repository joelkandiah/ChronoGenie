...
def plot_forecasts(results_dir, df, msoa_id, output_path, window_freq=None, sim_id=None, start_day=0, max_prediction_days=60, show_samples=False, num_samples_to_plot=20):
    """Replicate the reference plot style for Flow Matching."""
    ...
    colors = sns.color_palette("rocket", n_colors=len(windows)) # Different palette to distinguish from PM
    ...
    plt.suptitle(f"Flow Matching Forecasts - MSOA {msoa_id}", fontsize=16)
...
def plot_spaghetti(results_dir, msoa_id, window_start, sim_id, output_path, start_day=0, max_prediction_days=60):
    """Plot individual sample trajectories for a specific Flow Matching window."""
    ...
    axes[-1].set_xlabel("day")
    axes[0].legend()
    plt.suptitle(f"Flow Matching Spaghetti - MSOA {msoa_id}, Window {window_start}", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(output_path, dpi=300)
    print(f"Saved spaghetti plot to {output_path}")

def main():
    parser = argparse.ArgumentParser(description="Plot results from Flow Matching experiments.")
    parser.add_argument(
        "--results_dir",
        type=str,
        default=".",
    ...
    parser.add_argument("--output", type=str, default="flow_matching_forecast.png")
    parser.add_argument("--spaghetti_output", type=str, default="flow_matching_spaghetti.png")
    ...
    if args.window is not None:
        print(f"Plotting Flow Matching spaghetti for MSOA {args.msoa}, Sim {args.sim}, Window {args.window}...")
