        ...
        for sim_entry in tqdm(sim_dirs, desc=f"Loading {score_type} (use_log={use_log})"):
            ...
            with os.scandir(pred_root) as pred_it:
                for w_entry in pred_it:
                    ...
                    # Means are taken over the forecast horizons    
                    if score_type == 'energy':
                        ...
                        try:
                            # Read only 'msoa' and the specific CRPS column
                            table = feather.read_table(file_path, columns=['msoa', col_name])
                            df = table.to_pandas()
                            # Average over timesteps (all rows in this file for this MSOA)
...
def calculate_win_rates(df1, df2, df3=None, score_type='energy'):
    """
    Calculate the percentage of times each model performed best.
    """
    ...
    if score_type in ['energy', 'variogram']:
        ...
        on_cols = ['sim_id', 'msoa', 'window_start']
    ...
    # Determine which model won for each row
    ...
    win_rates = merged.groupby('window_start').agg(agg_dict).reset_index()
...
def plot_comparison(win_rates, title, xlabel, model_names, output_path):
    ...
    # Define styles for up to 3 models
    styles = [
        {'color': 'black', 'linestyle': '-', 'linewidth': 2},
        {'color': 'black', 'linestyle': '--', 'linewidth': 1.5},
        {'color': '#555555', 'linestyle': ':', 'linewidth': 2} # Dark gray dotted
    ]
    ...
    for i, name in enumerate(model_names):
        ...
        if col in win_rates.columns:
            ...
            plt.plot(win_rates['window_start'], win_rates[col], label=name, **style)
...
def main():
    parser = argparse.ArgumentParser(description="Compare two or three GENIE models using win-rate plots.")
    ...
    for use_log in [False, True]:
        ...
        for d, name in zip(model_dirs, model_names):
            ...
            if not df.empty:
                csv_name = str(output_root / f"{output_stem}_energy{log_prefix}_{name.replace(' ', '_')}.csv")
        ...
        for d, name in zip(model_dirs, model_names):
            ...
            if not df.empty:
                csv_name = str(output_root / f"{output_stem}_crps{log_prefix}_{name.replace(' ', '_')}.csv")
                ensure_parent_dir(csv_name)
                df.to_csv(csv_name, index=False)
                print(f"Saved scores to {csv_name}")
        ...
        for d, name in zip(model_dirs, model_names):
            ...
            if not df.empty:
                csv_name = str(output_root / f"{output_stem}_variogram{log_prefix}_{name.replace(' ', '_')}.csv")
                ensure_parent_dir(csv_name)
                df.to_csv(csv_name, index=False)
                print(f"Saved scores to {csv_name}")
