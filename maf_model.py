...
# Normalising Flow model using Zuko's MAF (Masked Autoregressive Flow)
# MAF is a more flexible autoregressive flow than RealNVP/Coupling-based NSF
# but slower to sample from as it requires sequential processing.


class MAFFlowModel(nn.Module):
    def __init__(self, hidden_layers, input_dim, output_dim,
                 prediction_distribution_types, scalers,
                 num_transforms=3, sigma=0.1, dropout=0.2,
                 activation=nn.GELU):
        """
        Normalising Flow Model using Zuko's MAF.

        Args:
            hidden_layers: List of hidden layer widths for the masked networks
            ...
            num_transforms: Number of autoregressive transform layers
            sigma: Noise level for dequantisation (unused by the flow itself,
                   kept for API consistency with FlowMatchingModel)
            dropout: Dropout for masked networks (unused by Zuko's built-in
                     architectures, kept for API consistency)
            activation: Activation function for the masked networks (default: nn.GELU)
        """
        super().__init__()
        ...
        # MAF is autoregressive by default in Zuko
    ...
    def training_step(self, context, x1_raw):
        ...
        Backpropagation runs through the MAF masked network parameters.
        """
        B, M, D = x1_raw.shape
