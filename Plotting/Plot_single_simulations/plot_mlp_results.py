...
def plot_forecasts(results_dir, df, msoa_id, output_path, window_freq=None, sim_id=None, start_day=0, max_prediction_days=60, show_samples=False, num_samples_to_plot=20):
    """Replicate the reference plot style for MLP results."""
    ...
    # Use a different color palette for MLP (Viridis or Deep)
    colors = sns.color_palette("viridis", n_colors=len(windows))
    ...
    for i, burden in enumerate(prediction_names):
        ...
        if gt_col in df_msoa.columns:
            gt_df = df_msoa[['timestep', gt_col]].drop_duplicates().sort_values('timestep')
        ...
        for w_idx, w_start in enumerate(windows):
            ...
            ax.axvline(x=w_start, color=color, linestyle='-', alpha=0.3, linewidth=1)
            ...
            if not show_samples:
                # 50% CI
                if f'{burden}_lower_50' in w_df.columns and f'{burden}_upper_50' in w_df.columns:
                ...
                # 95% CI
                if f'{burden}_lower_95' in w_df.columns and f'{burden}_upper_95' in w_df.columns:
                    ax.fill_between(w_df['timestep'], 
    ...
    plt.suptitle(f"MLP (DART) Forecasts - MSOA {msoa_id}", fontsize=16)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(output_path, dpi=300)
    print(f"Saved plot to {output_path}")

def main():
    parser = argparse.ArgumentParser(description="Plot results from MLP (DART) experiments.")
    parser.add_argument(
        "--results_dir",
        type=str,
        default=".",
    ...
    parser.add_argument("--output", type=str, default="mlp_forecast.png")
    ...
    print(f"Loading MLP results from {args.results_dir} (Filter: {args.sim})...")
